...
namespace WebAPI.Controllers.V1
{
    ...
    public class PostsController : ControllerBase
    {
        ...
        public async Task<IActionResult> Get([FromQuery] PaginationFilter paginationFilter, [FromQuery] SortingFilter sortingFilter)
        {
            ...
            var validSortingFilter = new SortingFilter(sortingFilter.SortField, sortingFilter.Aescending);

            var posts = await _postService.GetAllPostsAsync(validPaginationFilter.PageNumber, validPaginationFilter.PageSize,
                                                            validSortingFilter.SortField, validSortingFilter.Aescending);
            
            var totalRecord = await _postService.GetAllPostsCountAsync();
            
            return Ok(PaginationHelper.CreatePagedResponse(posts, validPaginationFilter, totalRecord));
        }
